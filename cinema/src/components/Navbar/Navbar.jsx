  ...
  return (
    <motion.section initial={{y:-100,opacity:0}} animate={{y:0,opacity:1}} transition={{duration:1}}  className={blur ? "navbar-container blur" : "navbar-container"}>
      ...
      <div className="nav-links" style={getMenuStyle(open)}>
        ...
        <NavLink to="/solution" className="nav-link" onClick={handleopen}>Solutions</NavLink>
        {/* <a href='https://www.linkedin.com/' target='_blank' className="nav-link" > <div style={{display:'flex',alignItems:'center', gap:'0.5rem',justifyContent:'center'}}><img src={images.linkedin} width={15} alt="" /> Blogs</div> </a> */}
        <NavLink to="/about" className="nav-link" onClick={handleopen}>About Us</NavLink>
        <NavLink to="/contact" className="nav-link contact-us" onClick={handleopen}>Contact Us</NavLink>
        <Link to="/contact" className="nav-links" onClick={handleopen}><button>Contact Us</button></Link>
